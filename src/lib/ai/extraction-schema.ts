...
TRUCK COMPLIANCE PLATE LOCATION: The compliance plate for Australian trucks is almost always riveted to the ENGINE BAY FIREWALL — the flat panel between the cab and the engine compartment. Open the bonnet and look at the firewall face. It is a rectangular metal plate with Make, Model, VIN/WMI, GVM, GCM, Tare, Axle loads, Date of Manufacture (in MM/YYYY format), and ADR compliance numbers. The compliance_date field extracts the MM/YYYY value from this plate. Always inspect engine bay photos for this plate.
...
- IMPORTANT — MULTIPLE PLATES: When multiple build plates are visible across photos, you MUST identify which plate belongs to the MAIN ASSET and which belong to attachments or components. Use these rules:
...
The Chassis Number (sometimes labelled "Chassis No" or "Frame No") is a shorter alphanumeric identifier stamped on the chassis rail or on the build plate — it is typically 8–15 characters and is separate from the VIN. The VIN (Vehicle Identification Number) is always exactly 17 characters. If you see both on a plate or in photos, extract each to its correct field: the 17-character value goes into the vin field, the shorter stamped chassis number goes into the chassis_number field. Do not put the VIN into chassis_number or vice versa. If a plate shows both a Chassis Number and a VIN, both should be extracted — do not discard either.
...
ODOMETER vs TRIP METER: The odometer shows TOTAL lifetime km — typically a large number (e.g. 186,450). The trip meter resets and shows a smaller number (0-9,999km) — it is labelled 'TRIP A', 'TRIP B', 'ODO-T', or similar. ALWAYS extract the large total km reading, never the trip meter. If both are visible, the larger number is always the odometer.
...
- FIVE DIGIT TRAP: If you read what appears to be 5 digits on a hourmeter (e.g. "12345"), STOP and recount. Truck and earthmoving hourmeters almost never exceed 25,000–30,000 hours in real service. A 5-digit reading likely means you have combined the main hours digits (4 large digits) with a separate tenths/decimal digit into one number. "12345" is almost certainly "1234" hours + ".5" tenths shown on a smaller sub-display or in a different colour/size. Recount: if removing the last digit gives a plausible reading (e.g. "1234"), the fifth digit is a decimal — return "1234.5" or "1234" (whichever matches the display format). Only accept a 5-digit reading if the display clearly shows 5 equally-sized digits with no visible separation.
- HOURS SANITY CHECK — MANDATORY: No truck, excavator, forklift, or agricultural machine ever accumulates more than 50,000 hours in service. If your extracted hourmeter reading exceeds 50,000, it is WRONG — you have misread the display. Common mistakes: reading a minutes counter (e.g. 312,455 minutes = ~5,207 hours), reading an odometer, or misidentifying which display is the hourmeter. If you get a reading over 50,000: reject it, return null, and note that the correct hours could not be determined from photos. Do not return implausible readings.
...
SERVICE HISTORY BOOK: If the inspector photographs a service logbook, maintenance record book, or service sticker/label, extract:
- service_history field: 'Full Service History — Log Books' if the book appears complete with multiple stamps, or 'Partial Service History' if only some stamps, or 'Service Records Present' if records are shown
- Look for the most recent service stamp or sticker showing the date and odometer/hours at last service — note in inspection extras if readable
...
- TRUCKS: infer engine_manufacturer, engine_series, engine_size, fuel_type, gearbox_make, transmission, drive_type, suspension, axle_configuration, brakes, GVM, GCM, fifth_wheel (prime movers only — infer brand from make if not visible in photos; AU standard fitments by make: Kenworth→Jost JSK 37 [standard], SAF-Holland FW35 [optional]; Volvo→Jost JSK 37 or SAF-Holland; Mack→Jost JSK 37 or SAF-Holland FW35; Scania→Jost JSK 37; Mercedes Actros→Jost JSK 37; Freightliner→Fontaine or Jost; Western Star→Jost or Fontaine), torque (peak engine torque — digits only in Nm, e.g. Cummins X15 = 2780, PACCAR MX-13 = 2400, Volvo D13 = 2550, Mack MP8 = 2508, Detroit DD15 = 2576).
  ...
  HINO 300 Series: 616→N04C-TI 4.0L 4-cyl 82kW (110hp); 617→N04C-TI 4.0L 4-cyl 96kW (129hp); 717→N04C-TQ 4.0L 4-cyl 110kW (147hp); 816→N04C-UN 4.0L 4-cyl 147kW (197hp); 921→N04C-UN 4.0L 4-cyl 147kW (197hp)
  ...
  ISUZU NLR/NPR/NQR (light-medium rigid): NLR 45-150→4JJ1 3.0L 4-cyl 110kW (147hp); NPR 55-155→4HK1 5.2L 4-cyl 110kW (147hp); NPR 65-155→4HK1 5.2L 4-cyl 110kW (147hp); NQR 87-190→4HK1 5.2L 4-cyl 140kW (188hp)
  ISUZU FRR/FSR/FVR/FVZ (medium-heavy rigid): FRR 107-210→6HK1 7.8L 6-cyl 156kW (209hp); FSR 140-260→6HK1 7.8L 6-cyl 206kW (276hp); FVR 165-300→6HK1 7.8L 6-cyl 228kW (306hp); FVZ 260-300→6UZ1 9.8L 6-cyl 257kW (345hp)
...
- EARTHMOVING: infer engine_manufacturer, engine_model, horsepower, fuel_type, drive_type, transmission (include brand where known — Cat own powershift, Komatsu PCSS, Komatsu HST, Volvo PT1851/PT1901, Doosan powershift), emissions_tier, operating_weight (stored in tare field), capacity (GP bucket capacity in m³ for excavators — e.g. Cat 320 = 0.9m³, Cat 330 = 1.2m³, Cat 349 = 2.1m³, Komatsu PC200 = 0.8m³, PC300 = 1.4m³, Hitachi ZX200 = 0.8m³, ZX330 = 1.4m³; bucket capacity in m³ for wheel loaders), track_type (infer from undercarriage/exterior photos or machine class: excavators and dozers >8t = "Steel", mini excavators ≤8t and CTLs = "Rubber"; null for wheeled machines), and torque_rpm (peak torque at rated RPM — format "X Nm @ Y rpm", e.g. Cat 320 = 1,450 Nm @ 1,400 rpm, Komatsu PC200 = 1,068 Nm @ 1,500 rpm, Cat 966 loader = 1,457 Nm). Operating weight examples: Cat 320 = 20,800kg, Cat 330 = 30,600kg, Komatsu PC200 = 20,100kg, Komatsu PC300 = 30,000kg; wheel loaders: Cat 950 = 18,500kg, Cat 966 = 24,000kg
  ...
  CAT EXCAVATORS: 308/308E2 → engine_manufacturer=Caterpillar, engine_model=C2.4, horsepower=66hp; 313/313GC → C3.3B, 95hp; 320D/320E/320F/320GC → C7.1, 148–153hp (most common AU excavator); 323 → C7.1, 162hp; 326/326GC → C7.1, 204hp; 325/325D → C7.1, 200–206hp; 330/330D → C9, 270hp; 330F/330GC → C9.3, 264–272hp; 335 → C7.1, 302hp; 336/336D → C9, 302hp; 336E/336F/336GC → C9.3, 308hp; 345 → C13, 397hp; 349/349D/349E/349F → C13, 394hp; 352/352F → C13, 394hp; 374/374D/374F → C15, 467–469hp; 390/390D/390F → C18, 523–532hp
  ...
  KUBOTA MINI EXCAVATORS (very common AU auction — always check these models): KX018-4 → engine_manufacturer=Kubota, engine_model=D722 3-cyl Diesel, horsepower=11.2hp, operating_weight=1800kg, track_type=Rubber; KX040-4 → Kubota V2403 4-cyl Diesel, 36hp, 4000kg, Rubber; KX057-4 → Kubota V3307 4-cyl Diesel, 55hp, 5700kg, Rubber; U48-4 → Kubota V2403 4-cyl Diesel, 36hp, 4800kg, Rubber (wide zero-tail-swing); U55-4 → Kubota V3307 4-cyl Diesel, 55hp, 5500kg, Rubber. NOTE: All Kubota mini excavators use rubber tracks; fuel_type=Diesel for all; emissions_tier=Tier 4 Final for all post-2014 units.
  ...
  EARTHMOVING COLOUR: Do NOT assume yellow/orange is the colour — these are brand colours (Cat=yellow, Komatsu=yellow/blue, Hitachi=orange). Only extract colour if the machine is painted non-standard (e.g. a black excavator, a white dozer). For standard brand colours, leave the colour field empty — it is not useful data.
...
- FORKLIFTS: Use Make + Model + Year to infer the following fields when not directly readable from the data plate.
  ...
  TOYOTA FORKLIFT MODEL LOOKUP (Toyota is the most common AU forklift brand — apply this table whenever Make=Toyota):
  8FG15 → 1.5t LPG, Toyota 4Y engine, 68HP (50kW), pneumatic tyres, counterbalance
  8FG18 → 1.8t LPG, Toyota 4Y engine, 68HP (50kW), pneumatic tyres, counterbalance
  8FG25 → 2.5t LPG, Toyota 4Y engine, 68HP (50kW), pneumatic tyres, counterbalance (most common AU forklift)
  8FG30 → 3.0t LPG, Toyota 4Y engine, 68HP (50kW), pneumatic tyres, counterbalance
  8FD25 → 2.5t Diesel, Toyota 1DZ-II engine, pneumatic tyres, counterbalance
  8FBE15 → 1.5t Electric, electric motor, cushion or pneumatic tyres, counterbalance
  All Toyota 8-series forklifts: fuel from model prefix (FG=LPG, FD=Diesel, FBE/FB=Electric); capacity from last 2 digits × 100kg (15=1500kg, 18=1800kg, 25=2500kg, 30=3000kg); max_lift_capacity inferred at high confidence from model number.
...
- MARINE: infer hull_material from visual (fibreglass/aluminium most common), motor_type from photo (outboard vs inboard), number_of_engines from visible motors, steering_type from helm setup. Read beam, draft, and LOA from the hull compliance plate (transom plate or builder's plate riveted to the upper starboard transom or inside the cabin — this plate shows LOA, beam, draft, and persons capacity). Fuel capacity: look for the fuel cap label on deck (often stamped with litres), console instrument label, or compliance plate — output as "XL" e.g. "220L".
  ...
  MARINE MAKE/MODEL IDENTIFICATION — Australian small boat brands by category:
  - ALUMINIUM TINNIES AND PLATE BOATS (most common AU auction boats, typically 3–5m, outboard motor): Quintrex (most common — red/white decals, model range: Renegade, Hornet, Fishabout, Top Ender, Blade, Territory, Explorer, Dory, Escape), Stacer (blue/white decals, model range: Nomad, Crossfire, Seahorse, Assault, Baymaster, Wild Rider, Pro Angler, Pro Fisher), Savage (black/red decals, model range: Arrow, Renegade, Hornet), Horizon (silver/blue, model range: Series models), Stessl (Australian WA brand, silver/grey), Bluefin (silver/blue), Polycraft (heavy-duty polyethylene hull — distinctive moulded appearance, not riveted), Goldfish (WA brand), Brooker (QLD brand), Bowtech (fibreglass/aluminium), Blueline (plate aluminium), Ally Craft (bare aluminium finish), Sea Jay (SA brand, blue/silver). Read make from transom badge or hull decal — if no badge visible, infer from hull shape and colour scheme.
  - FIBREGLASS RUNABOUTS AND CABIN BOATS (typically 4–7m, outboard or sterndrive): Haines Hunter (most common AU fibreglass runabout — distinctive stepped hull, model range: Prowler, SF, SR, SL, SC series, V17, V19, V21, V23, V25), Whittley (Victorian builder, model range: Voyager, Sea Legend, CR series), Caribbean (model range: Utopia, Reef Runner), Cruise Craft (distinctive yellow/white gelcoat), Mustang (Australian fibreglass brand), Haines Signature (premium arm of Haines), Challenger (older AU brand), Bayliner (US brand, common on AU market, model range: 175, 185, 195, 215), Chaparral (US brand, model range: 18, 19, 21, 23 H2O series), Formosa (older AU/Taiwan brand).
  - JET SKIS / PERSONAL WATERCRAFT: Sea-Doo (Bombardier — red/black/white hulls, model range: Spark, GTI, GTX, RXT, RXT-X, Wake Pro, Fish Pro; read exact model from hull decal), Yamaha WaveRunner (blue/white/black hulls, model range: EX, VX, FX Cruiser, FX HO, SuperJet, FZR, FZS; read from hull decal), Kawasaki Jet Ski (green/black hulls, model range: STX-15F, STX-160, Ultra 310X, Ultra 310LX; note "Jet Ski" is a Kawasaki brand name). Always read model from hull decal — model appears on the hull side near the bow. Note number of seats (1-seater/stand-up, 2-seater, 3-seater).
  - YACHTS AND SAILBOATS: identify by presence of mast (aluminium spar rising vertically from deck). Common AU brands: Beneteau (French, model range: Oceanis, First, Sense), Bavaria (German, model range: Cruiser, Vision, Match), Jeanneau (French, model range: Sun Odyssey, Sun Fast), Catalina (US), Island Packet, Hanse, Hunter Marine. Read make and model from transom badge. Note rig type if identifiable: Sloop (one mast, mainsail + headsail), Ketch (two masts). Subtype: yacht on road trailer (<8m) = trailer_boat; larger moored yacht = commercial_vessel.

  HIN (HULL IDENTIFICATION NUMBER) — SPECIAL READING RULES:
  - HIN is a 12-character code stamped or moulded into the upper starboard transom (right side when looking from behind the boat). It uniquely identifies the hull.
  - Standard ABYC format: first 3 chars = manufacturer code (e.g. "QNX" for Quintrex, "STR" for Stacer), next 5 chars = hull serial (alphanumeric), next 2 chars = model year (encoded), last 2 chars = actual build year.
  - Example format: ABC12345D607 (manufacturer code + serial + month/year suffix).
  - Never infer or guess a HIN — only extract if the characters are directly legible in a photo. If partially obscured, extract whatever is visible.
...
- VEHICLES: infer engine_type, fuel_type, transmission, drive_type from make/model/year knowledge. Read VIN from door jamb plate (driver or passenger side) or windscreen base — 17-character alphanumeric, often stamped or on an adhesive label. Read rego plate number from exterior photos showing the front or rear plate — Australian state plates are rectangular, alphanumeric; read exactly as shown (e.g. "ABC-123", "T123-AB"). Read odometer from instrument cluster — extract exact digits only, return null if any digit is unclear. Identify body type from exterior shape. Identify colour from exterior paint including finish qualifier (Metallic, Pearl, Matte) where distinguishable — e.g. "Pearl White", "Metallic Silver", "Graphite". For extras, actively scan interior photos for: alloy wheels, sunroof, leather seats, heated seats, reverse camera, parking sensors, Apple CarPlay/Android Auto (visible on infotainment screen or dash), wireless charging pad, premium audio (Bose/JBL/Harman badges), blind-spot monitoring, adaptive cruise control. Scan exterior photos for: tow bar, bull bar, nudge bar, canopy/tray top, side steps/running boards, roof rack, snorkel, winch, aftermarket wheels, spot lights, UHF antenna (chrome whip on roof, bull bar, or cab corner). Read brand from badge or decal — common AU aftermarket brands: bull bars (ARB, Ironman 4x4, TJM, Opposite Lock, MCC/Metal Tech), canopies (Truckman, Aeroklas, MTM, Flexiglass, Tub Tector, Max Top), snorkels (ARB, Safari Snorkel, Ironman 4x4), winches (ARB, Warn, Runva, Ironman 4x4), roof racks (Rhino Rack, Thule, Prorack), side steps (ARB, Prorack, Safari). Always name the brand in extras if readable.
  ...
  - UHF ANTENNA: Look for a chrome whip antenna mounted on the roof, bull bar, or rear corner of the cab. For trucks, check the cab corners and sunvisor bracket. State 'UHF Antenna' in extras — never just 'UHF'.
